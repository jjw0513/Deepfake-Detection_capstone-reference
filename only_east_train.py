...
# Áö½Ä Áõ·ù¸¦ À§ÇÑ KL Divergence Loss
def knowledge_distillation_loss(student_outputs, teacher_outputs, labels, alpha, temperature):
    """
    student_outputs: Student ¸ðµ¨ÀÇ Ãâ·Â (·ÎÁþ °ª)
    teacher_outputs: Teacher ¸ðµ¨ÀÇ Ãâ·Â (·ÎÁþ °ª)
    labels: Ground truth ·¹ÀÌºí
    alpha: Ground truth¿Í Teacher ¿¹Ãø °£ °¡ÁßÄ¡ ºñÀ²
    temperature: Softmax¸¦ Àû¿ëÇÒ ¶§ÀÇ ¿Âµµ
    """
    soft_targets = nn.functional.softmax(teacher_outputs / temperature, dim=1)
    student_logits = nn.functional.log_softmax(student_outputs / temperature, dim=1)
    distillation_loss = nn.functional.kl_div(student_logits, soft_targets, reduction="batchmean") * (temperature ** 2)
    classification_loss = nn.functional.cross_entropy(student_outputs, labels)
...
def main():
    ...
    # Teacher ¸ðµ¨ ÁØºñ (µ¿°á)
    teacher_model = model_selection(modelname='xception', num_out_classes=2, dropout=dropout).to(device)
    teacher_model.load_state_dict(torch.load(model_path), strict=True)
    teacher_model.eval()

    # Student ¸ðµ¨ ÃÊ±âÈ­
    student_model = model_selection(modelname='xception', num_out_classes=2, dropout=dropout).to(device)
    student_model = nn.DataParallel(student_model)

    # Æ¯Á¤ ·¹ÀÌ¾î µ¿°á (conv1¸¸ µ¿°á)
    ...
    for fold, (train_idx, val_idx) in enumerate(kfold.split(full_dataset)):
        ...
        for epoch in range(epoches):
            ...
            for inputs, labels in train_loader:
                ...
                with torch.amp.autocast(device_type='cuda'):
                    ...
                    loss = knowledge_distillation_loss(student_outputs, teacher_outputs, labels, alpha=0.5, temperature=4)
        ...
        torch.save(student_model.module.state_dict(), os.path.join(output_path, f"best_model_fold_{fold + 1}.pkl"))
...
if __name__ == '__main__':
    ...
    parse.add_argument('--name', '-n', type=str, default='deepfake_xception_last3')
